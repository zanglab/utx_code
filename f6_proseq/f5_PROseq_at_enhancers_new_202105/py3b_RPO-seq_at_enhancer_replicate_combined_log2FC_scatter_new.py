...
outdir = 'f3b_RPO-seq_at_enhancer_replicate_combined_log2FC_scatter'
...
kept_df = pd.read_csv('data/overlapped/PROseq_on_H3K27ac_H3K4me1_with_UTX.bed',index_col=3,sep='\t',header=None)

proseq_RPKM_df = proseq_RPKM_df.loc[kept_df.index]
...
for ii in np.arange(len(compr_pairs)-1)[:]:
            ...
            # idx = z.argsort()
            # x, y, z = x[idx], y[idx], z[idx]
            # g = plt.scatter(x,y,c=z,cmap = plt.cm.Greens,s=6,rasterized=True)
