...
def sub_heatmap_plot(proseq_binding_df,gs,loc,project_dir,proseq_name,norm_pattern):
    ...
    ax.set_title('{}'.format(cellType_labels[proseq_names_title[proseq_name]]),fontsize=15)
    xp = g.get_xticks()
    ax.set_xticks([xp[0],xp[int(len(xp)/2)],xp[-1]])
    ax.set_xticklabels(['-2kb','0','2kb'],rotation=0,fontsize=13)
...
outdir = 'f5_heatmap_RPO-seq_at_enhancer_combi_rep_WT_changed_rep_Promoter_overlap'
...
proseq_names_title ={'PCDH':'Vector',
                     ...
                     'FL':'WT',}
norm_pattern='RPKM'
prename = 'H3K27ac_H3K4me1_with_UTX'
norm_col='total_dm6_f0x2'

diff_types = {'increased':[operator.gt,1],
             'decreased':[operator.lt,-1]}
promoter_overlap_types = ['promoter_overlapped','promoter_NON_overlapped']
...
for promoter_overlap_type in promoter_overlap_types[:]:
    id_file = 'data/overlapped/PROseq_on_H3K27ac_H3K4me1_with_UTX_{}.bed'.format(promoter_overlap_type)
    promoter_overlap_type_df = pd.read_csv(id_file,sep='\t',header=None,index_col=3)
    for diff_type in diff_types.keys():
        for diff_thre in [1,1.2][:]:
            # increased or decreased in WT over Vector
            ...
            # overlapped with promoter or not
            # kept_df = kept_df.loc[kept_df.index.intersection(promoter_overlap_type_df.index)].dropna()
            kept_df = kept_df.loc[kept_df.index.intersection(promoter_overlap_type_df.index)].dropna()
            # plot the heatmap
            composite_data = {} # signal for composite plot
            for proseq_name in proseq_names[:]:
                ...
                # keep only those with changed signals
                proseq_binding_df = proseq_binding_df.loc[kept_df.index].dropna()
                ...
                plt.savefig(outdir+os.sep+'WT_{}_{}_heatmap_{}_binding_by_{}_FC{}_{}.png'.format(\
                            diff_type,norm_pattern,proseq_names_title[proseq_name],prename,diff_thre,promoter_overlap_type),bbox_inches='tight',pad_inches=0.1,transparent=True,dpi=600)
                plt.show()
                plt.close()
            ...
            plt.ylabel('RPKM')
            plt.title('RPO-seq')
            # plt.title('{} FC{} \n{}'.format(diff_type,diff_thre,promoter_overlap_type),fontsize=11)
            ...
            plt.legend(fontsize=12,borderaxespad=0.1,labelspacing=.2,handletextpad=0.2,
                    ...
                    frameon=False)
            plt.savefig(outdir+os.sep+'WT_{}_composite_PROseq_binding_by_{}_at_{}_FC{}_{}.png'.format(diff_type,norm_pattern,prename,diff_thre,promoter_overlap_type),\
                        bbox_inches='tight',pad_inches=0.1,transparent=True,dpi=600)
            plt.show()
            plt.close()
            
            kept_df.to_csv(outdir+os.sep+'WT_{}_FC{}_{}.csv'.format(diff_type,diff_thre,promoter_overlap_type))
