...
def return_rep_combined_df(project_dir,proseq_name,rep_id):
    rep_name='{}{}'.format(proseq_name,rep_id)
    proseq_binding_file = '{}/f6_proseq/data_0x2_MAPQ10/f5_proseq_union_peak_RPKM/{}_on_union_Proseq_peak_es2kb.csv'.format(project_dir,rep_name)
...
outdir = 'f3c_RPO-seq_at_enhancer_replicate_combined_log2FC_scatter_sep_promoter_overlap'
...
for promoter_overlap_type in promoter_overlap_types[:]:    
    ...
    for proseq_name in proseq_names[:]:
            ...
            proseq_binding_df1 = return_rep_combined_df(project_dir,proseq_name,'1PRO')
            proseq_binding_df2 = return_rep_combined_df(project_dir,proseq_name,'2PRO')
            ...
            proseq_RPKM_df['PROseq_RPKM_{}'.format(proseq_names_title[proseq_name])] = proseq_binding_df['RPKM']
    ...
    # ==== select promoter overlapped and non-overlapped regions
    kept_df = pd.read_csv('data/overlapped/PROseq_on_H3K27ac_H3K4me1_with_UTX_{}.bed'.format(promoter_overlap_type),index_col=3,sep='\t',header=None)
    proseq_RPKM_df = proseq_RPKM_df.loc[kept_df.index];print(proseq_RPKM_df)
    proseq_RPKM_df.to_csv(outdir+os.sep+'differential_PROseq_RPKM_{}.csv'.format(promoter_overlap_type))
    ...
    for ii in np.arange(len(compr_pairs)-1)[:]:
            ...
            output_prename = 'PROseq_RPKM_{}_over_{}_vs_{}_over_{}_{}'.format(treatment_x,control_x,treatment_y,control_y,promoter_overlap_type)
            ...
            # z[np.where(np.isnan(z))] = 0.0
            # idx = z.argsort()
            # xnew, ynew, z = x[idx], y[idx], z[idx]
            # g = plt.scatter(x,y,c=z,cmap = plt.cm.GnBu_r,s=6,rasterized=True)
            plt.scatter(x,y,rasterized=True,s=6,c='k')
            plt.axhline(y=0,c='gray',lw=.8,ls='--')
            plt.axvline(x=0,c='gray',lw=.8,ls='--')
            plt.xlabel(' {} over {} '.format( cellType_labels[treatment_x],cellType_labels[control_x]))
            plt.ylabel(' {} over {} '.format( cellType_labels[treatment_y],cellType_labels[control_y]))
            plt.title('$\Delta$PRO-seq')
            x_sort = np.sort(x)
            plt.plot(x_sort,x_sort*slope+intercept,c = 'k',ls='--',lw=.9)
            plt.text(.97,.97,'$r={:.2f}$'.format(r_value),fontsize=13,transform=plt.axes().transAxes,ha='right',va='top')
