...
def plot_figs(box_vals,outdir,compr_type,dis):
    ...
    plt.axes().set_xticklabels(['All genes (n$=${})'.format(len(box_vals[0])),\
                                'H3K4me1 $\pm$ {}kb (n = {})'.format(str(int(dis/1000)),len(box_vals[1])),\
                                'H3K27ac $\pm$ {}kb (n = {})'.format(str(int(dis/1000)),len(box_vals[2]))],rotation=30,ha='right')    
    ...
    plt.savefig('{}/dis{}bp_compr_{}.png'.format(outdir,dis,compr_type,),bbox_inches='tight',pad_inches=0.1,dpi=600,transparent=True)
    plt.show()
    plt.close()
    
    
def binding_nearby_gene_log2FC(overlap_file,deg_df):
    overlap_df = pd.read_csv(overlap_file,sep='\t',index_col=4)
    overlapped_genes = overlap_df[overlap_df['IfOverlap']==1]['overlapped_genes']
...
for compr_type in compr_types[:]:
    ...
    for dis in extend_dis[:]:
        k4_file = 'f1_HM_binding_promoter_overlap/H3K4me1_peaks_tss_es{}bp.csv'.format(dis)
        k27_file = 'f1_HM_binding_promoter_overlap/H3K27ac_peaks_tss_es{}bp.csv'.format(dis)
        k4_FC =  binding_nearby_gene_log2FC(k4_file,deg_df)
        k27_FC =  binding_nearby_gene_log2FC(k27_file,deg_df)
        all_log2FC =  deg_df['log2FoldChange'].dropna()
        box_vals = [all_log2FC,k4_FC,k27_FC]
        plot_figs(box_vals,outdir,compr_type,dis)
        k4_FC.to_csv('{}/_k4me1_dis{}bp_compr_{}_log2FC.csv'.format(outdir,dis,compr_type))
        k27_FC.to_csv('{}/_k27ac_dis{}bp_compr_{}_log2FC.csv'.format(outdir,dis,compr_type))
