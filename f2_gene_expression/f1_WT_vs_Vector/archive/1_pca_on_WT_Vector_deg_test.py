...
#     print(len(upgenes.union(dngenes)));exit()
    ...
    df = df.loc[dngenes.union(upgenes)].dropna()
    print('df dim\t',df.shape)#;exit()
...
#     pca_with_tsne(np.transpose(df),df.columns,label_colors,figname,re_order,re_labels,mark_text=False)
    ...
    print(df.columns,df.index[:10])
    #df = log_quantile_normalization(df)
#     figname = '{}/tpm_PCA_QN.pdf'.format(outdir)
#     pca_plot(np.transpose(df),df.columns,label_colors,figname,re_order,re_labels,mark_text=True)

    df['UTX_FUSIDR_rep2']=10*df['UTX_FUSIDR_rep2']
    df['Vector_rep2']=10*df['Vector_rep2']
    figname = '{}/tpm_PCA_seqDepth.pdf'.format(outdir)
    pca_plot(np.transpose(df),df.columns,label_colors,figname,re_order,re_labels,mark_text=True)
