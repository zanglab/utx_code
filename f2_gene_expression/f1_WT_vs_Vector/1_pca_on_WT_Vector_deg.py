...
#     df = np.log2(df+1)
    ...
    # PCA of all genes
    df = log_quantile_normalization(df_ori)
    print('df dim\t',df.shape)#;exit()
    figname = '{}/tpm_PCA_all_genes.pdf'.format(outdir)
    pca_plot(np.transpose(df),df.columns,label_colors,figname,re_order,re_labels,mark_text=False)
    
    
    # PCA of DEG between WT and Vector
    df = df_ori.loc[dngenes.union(upgenes)].dropna()
    df = log_quantile_normalization(df)
    print('df dim\t',df.shape)#;exit()
    figname = '{}/tpm_PCA_DEG_of_WT_over_Vector.pdf'.format(outdir)
    pca_plot(np.transpose(df),df.columns,label_colors,figname,re_order,re_labels,mark_text=False)
