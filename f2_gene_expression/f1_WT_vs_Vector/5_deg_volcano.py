...
#     file_dir='/Volumes/zanglab/zw5j/since2019_projects/UTX_HaoJiang/f0_data_process/rna_seq/data_1st_submit_STAR_RSEM_new/'
    ...
    infiles=[deseq2_file1,deseq2_file2]
    for infile in infiles:
        df = pd.read_csv(infile,sep=',',index_col=0)
        df = df.dropna()
        basename = os.path.basename(infile).split('.csv')[0]
#         figname='{}/{}_baseMean_log2FC.pdf'.format(outdir,basename)
#         columns = ['baseMean','log2FoldChange']
#         xlabel,ylabel = 'log2(baseMean)','log2FC'
#         scatter_all_chip_binding(df,columns,figname,xlabel,ylabel)
    
        figname='{}/{}_pvalue_log2FC.pdf'.format(outdir,basename)
        columns = ['log2FoldChange','padj']
        xlabel,ylabel = 'log2FC','-log$_{{10}}$(adj.p)'
        scatter_all_chip_binding_2(df,columns,figname,xlabel,ylabel,adjp,logfc,basename)
