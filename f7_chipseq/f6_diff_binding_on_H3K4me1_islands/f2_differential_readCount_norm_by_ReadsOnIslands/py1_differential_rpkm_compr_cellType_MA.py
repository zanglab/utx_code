...
def return_chipseq_rpkm_csv(celltype,factor,genomic_region,norm_col):
    ...
    file_dir='{}/f7_chipseq/f6_diff_binding_on_H3K4me1_islands/data_RPKM/rpkm_csv'.format(project_dir)
    csv_file='{}/{}_{}_on_H3K4me1_islands{}.csv'.format(file_dir,celltype,factor,genomic_region)
...
def return_differential_score(df,treatment,control,factor,genomic_region,outdir,norm_col):
    # == log2FC
...
#     plt.scatter(x,y,rasterized=True,s=7,c='k')
    ...
    plt.savefig(outdir+os.sep+'{}_{}_{}_{}_MA.pdf'.format(norm_col,factor,genomic_region,add_col),bbox_inches='tight',pad_inches=0.1,dpi=600,transparent=True)
    plt.show()
    plt.close()
    return df
...
factors = ['UTX','UTXFEB','MLL4','H3K4me1','H3K4me2','H3K4me3','H3K27ac','H3K27me3']
genomic_regions = ['_es500bp','_es2kb','']
norm_cols = ['total','total_in_islads']

outdir='f1_differential_binding_MA'
...
for factor in factors[:]:
    for genomic_region in genomic_regions[:]:
        for norm_col in norm_cols[:]:
            ...
            df = return_differential_score(df,'WT','Vector',factor,genomic_region,outdir,norm_col)
            df = return_differential_score(df,'DEL','WT',factor,genomic_region,outdir,norm_col)
            df = return_differential_score(df,'EIF','DEL',factor,genomic_region,outdir,norm_col)
            # print(celltype,factor,df);exit()
            df.to_csv(outdir+os.sep+'_{}_{}_{}_differential_RPKM.csv'.format(norm_col,factor,genomic_region))
