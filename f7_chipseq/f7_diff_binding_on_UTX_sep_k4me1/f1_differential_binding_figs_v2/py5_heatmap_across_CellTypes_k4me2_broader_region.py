...
def prepare_each_subfig(df_tmp,gs,heatmap_pos,peak_file,factor,celltype,genomic_region):
    ...
    ax.set_xticklabels(['-{}'.format(genomic_region),'{}'.format(genomic_region)],rotation=45,fontsize=13)
    ax.set_title('{}\n{}'.format(factor[:-3], cellType_labels[celltype]),fontsize=14)
    return df.mean()
    ...
    for peak_file in peak_files[:]:
        ...
        for factor in factors[:]:
            ...
            for celltype in cellTypes[:]:
                avg_binding = prepare_each_subfig(master_df,gs,heatmap_pos,peak_file,factor,celltype,genomic_region)
            ...
            plt.ylabel('{} signal'.format(factor[:-3]))
            # plt.ylim(ymax=9 if norm_pattern=='RPKM' else 5)
            plt.axes().set_xticks([0,100,200])
            plt.axes().set_xticklabels(['-{}'.format(genomic_region),'0','{}'.format(genomic_region)])
