...
indir='../f0_data_integration/f3_combined_data_RPKM/'
outdir='f2b_diff_logFC_per_CellType_compr_Factor_scatter_k4me1_increase'
...
for peak_file in peak_files[:]:
    ...
    for fc_thre in fc_thres:
        df_tmp = df[(df[k4me1_log2fc_col]> np.log2(fc_thre)) & (df[k4me1_log2avg_col]>log2avg_thre)] 
        ...
        for compr_pair in compr_pairs[:1]:
            ...
            for antibody_pair in antibody_pairs[:]:
                ...
                x = df_tmp[factor1_logFC]
                y = df_tmp[factor2_logFC]
                ...
                plt.savefig(outdir+os.sep+'fcthre_{}_{}_{}_vs_{}_compr_{}_over_{}.pdf'.format(fc_thre,peak_file,antibody_pair[0],antibody_pair[1],compr_pair[0],compr_pair[1]),bbox_inches='tight',pad_inches=0.1,dpi=600,transparent=True)
                plt.show()
                plt.close()
