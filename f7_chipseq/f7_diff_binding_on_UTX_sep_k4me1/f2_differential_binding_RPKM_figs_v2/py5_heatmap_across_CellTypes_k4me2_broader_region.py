...
def prepare_each_subfig(df_tmp,gs,heatmap_pos,peak_file,factor,celltype,genomic_region,factor_cellTypes_num):
    # read the binding pattern for each factor/celltype
    csv_file='../data_binding_patter_readCount/readCount_csv_k4me2APR_broader_regions/{}_{}_on_{}_es{}_bin200.csv'.format(celltype,factor,peak_file,genomic_region)
    ...
    norm_kb_factor = int(genomic_region[:-2])*2/200 # genomic_region*2 into 200 bins, per kb
    df = df/(norm_factor*norm_kb_factor) #  per kb per million mapped reads
    ...
    if not heatmap_pos==factor_cellTypes_num:
        cbar.remove()
    xp = g.get_xticks()
    ax.set_xticks([xp[0],xp[-1]])
    ax.set_xticklabels(['-{}'.format(genomic_region),'{}'.format(genomic_region)],rotation=45,fontsize=10)
    ax.set_title('{}\n{}'.format(factor[:-3], cellType_labels[celltype]),fontsize=10)
    return df.mean()
...
outdir = 'f5_heatmap_across_CellTypes_H3K4me2'
...
genomic_regions = ['10kb','20kb','50kb']

for genomic_region in genomic_regions[1:2]:
    for peak_file in peak_files[:]:
        ...
        for factor in factors[-1:]:
            ...
            for celltype in factor_cellTypes[:]:
                avg_binding = prepare_each_subfig(master_df,gs,heatmap_pos,peak_file,factor,celltype,genomic_region,factor_cellTypes_num)
                composite_data[celltype]=avg_binding
                heatmap_pos+=1
            plt.savefig(outdir+os.sep+'{}_{}_binding_es{}.png'.format(peak_file,factor,genomic_region),bbox_inches='tight',pad_inches=0.1,transparent=True,dpi=600)
            plt.show()
            plt.close()                
            ...
            plt.ylabel('RPKM'.format(factor[:-3]),fontsize=13)
            plt.title(factor[:-3],fontsize=13)
            # plt.ylim(ymax=9 if norm_pattern=='RPKM' else 5)
            plt.yticks(fontsize=11)
            plt.axes().set_xticks([0,100,200])
            plt.axes().set_xticklabels(['-{}'.format(genomic_region),'0','{}'.format(genomic_region)],fontsize=12)
            plt.legend(fontsize=10,borderaxespad=0.1,labelspacing=.2,handletextpad=0.2,
                       handlelength=1,loc="upper right",
                       bbox_to_anchor=[1.01,.5],
                       frameon=False)
            plt.savefig(outdir+os.sep+'composite_{}_{}_binding_es{}.png'.format(peak_file,factor,genomic_region),bbox_inches='tight',pad_inches=0.1,transparent=True,dpi=600)
            plt.show()
            plt.close()
