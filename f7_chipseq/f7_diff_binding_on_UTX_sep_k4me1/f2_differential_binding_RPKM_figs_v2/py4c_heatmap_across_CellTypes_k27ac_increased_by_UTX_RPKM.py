...
outdir = 'f4c_heatmap_across_CellTypes_k27ac_increased_by_UTX_RPKM'
...
# kept_index = [i for i in index_df.index if not i.startswith('chrUn')]
...
for avg_thre in avg_thres[:]:
    for peak_file in peak_files[:]:
        ...
        ranked_index = master_df.UTX_WT.sort_values(ascending=False).index
        master_df = master_df.loc[ranked_index]
        # master_df = master_df[(master_df[hm_log2fc_col]> np.log2(fc_thre)) & (master_df[hm_log2avg_col]>log2avg_thre)] 
        master_df = master_df[(master_df[hm_log2fc_col]> np.log2(fc_thre)) & (master_df[hm_log2avg_col]>np.log2(avg_thre))] 
