...
peak_files = ['H3K4me1','H3K27ac','H3K27me3']
    ...
    for factor in factors[:]:
        ...
        for ii in np.arange(len(compr_pairs)-1)[:]:
            ...
            if factor_logFC_y in df.columns:
                ...
                plt.xlabel('{} over {}'.format( cellType_labels[treatment_x],cellType_labels[control_x]),fontsize=14)
                plt.ylabel('{} over {}'.format( cellType_labels[treatment_y],cellType_labels[control_y]),fontsize=14)
                ...
                plt.text(.975,.97,'$r={:.2f}$ '.format(r_value),fontsize=12,transform=plt.axes().transAxes,ha='right',va='top')
