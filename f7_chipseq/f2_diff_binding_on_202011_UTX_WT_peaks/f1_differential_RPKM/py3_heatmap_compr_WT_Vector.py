...
def prepare_each_subfig(project_dir,factor,celltype,newindex,loc,genomic_region,cbar_ymax=False):
...
for genomic_region in genomic_regions:
    for factor in factors[:]:    
        ...
        cbar_ymax = prepare_each_subfig(project_dir,factor,'WT',newindex,1,genomic_region)
        cbar_ymax = prepare_each_subfig(project_dir,factor,'Vector',newindex,0,genomic_region,cbar_ymax)
        # loc+=1
                
        plt.savefig(outdir+os.sep+'{}_{}.png'.format(factor,genomic_region),bbox_inches='tight',pad_inches=0.1,transparent=True,dpi=600)
        plt.show()
        plt.close()
