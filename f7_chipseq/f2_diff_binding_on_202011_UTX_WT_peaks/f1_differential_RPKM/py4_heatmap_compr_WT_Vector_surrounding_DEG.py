...
def sub_heatmap_plot(df,loc,factor,celltype,deg_type):
    
    pal = sns.light_palette('r',as_cmap=True)   
    ...
    if loc==0:
        ax.set_ylabel('{} binding \n at {} genes'.format(factor,deg_type)) 
        cbar.remove()
    xp = g.get_xticks()
...
def prepare_each_subfig(project_dir,factor,celltype,newindex,loc,genomic_region,deg_type):
    ...
    df = df.loc[newindex]
    sub_heatmap_plot(df,loc,factor,celltype,deg_type)
    ...
    for genomic_region in genomic_regions:
        for factor in factors[:]:    
            ...
            for celltype in ['Vector','WT']:
                prepare_each_subfig(project_dir,factor,celltype,newindex,loc,genomic_region,deg_type)
                loc+=1
                    
            plt.savefig(outdir+os.sep+'{}_{}_surrounding_{}genes.png'.format(factor,genomic_region,deg_type),bbox_inches='tight',pad_inches=0.1,transparent=True,dpi=600)
            plt.show()
            plt.close()
