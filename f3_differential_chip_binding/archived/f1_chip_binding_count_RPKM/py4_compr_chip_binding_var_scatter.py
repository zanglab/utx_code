...
def compr_plot(df1,df2,compr_col,marker_pair,rpkm_pattern,outdir,genes,color,label):
...
#     t = plt.scatter(x[dn_genes],y[dn_genes],s=4,c='r',rasterized=True,alpha=1,label='Vector up')
    ...
    figname = '{}/{}_vs_{}_{}_{}.png'.format(outdir,marker_pair[0],marker_pair[1],rpkm_pattern,'_'.join(label.split(' ')))
    plt.show()
...
outdir = 'f4_compr_chip_binding_figs'
os.makedirs(outdir,exist_ok=True)

deg_dir='../../f0_data_process/rna_seq/data_1st_submit_STAR_RSEM_new/f6_deg/f2_deg/'
...
for marker_pair in marker_pairs[:1]:
    for rpkm_pattern in rpkm_patterns[:1]:
        csv_file1='f3_highly_variable_chip_binding/{}_{}_TPM_residual.csv'.format(marker_pair[0],rpkm_pattern)
        csv_file2='f3_highly_variable_chip_binding/{}_{}_TPM_residual.csv'.format(marker_pair[1],rpkm_pattern)
        ...
        compr_col='residual'
        compr_plot(df1,df2,compr_col,marker_pair,rpkm_pattern,outdir,up_genes,'royalblue','Vector down')
        compr_plot(df1,df2,compr_col,marker_pair,rpkm_pattern,outdir,dn_genes,'r','Vector up')
