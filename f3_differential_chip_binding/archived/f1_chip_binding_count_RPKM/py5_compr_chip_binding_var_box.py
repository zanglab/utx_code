...
def compr_plot(high_var_df,chip_marker,rpkm_pattern,outdir,):
    ...
    box_vals = []
    for i in positions:
        box_vals.append(np.log2(high_var_df[columns[i]].values))
    ...
    plt.ylabel('{} level at {} genes'.format(chip_marker,high_var_df.shape[0]),fontsize=17)
...
outdir = 'f5_compr_chip_binding_box'
...
for chip_marker in chip_markers[:]:
    for rpkm_pattern in rpkm_patterns[:]:
        csv_file='f3_highly_variable_chip_binding/{}_{}_TPM_residual.csv'.format(chip_marker,rpkm_pattern)
        df = pd.read_csv(csv_file,index_col=0)
        # == select high-var genes
        high_var_df = df[df['residual_pvalue']<0.01]
        compr_plot(high_var_df,chip_marker,rpkm_pattern,outdir,)
