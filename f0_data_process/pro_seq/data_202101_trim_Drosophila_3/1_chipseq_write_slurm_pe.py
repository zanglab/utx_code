...
def write_fq_process(outname,slurmout,genome_index,splitfile1,splitfile2,fq_base,fqdir,outdir,aln=False):
    ...
    saifile1 = outdir+os.sep+outname+'_'+fq_base+'.1.sai'
    saifile2 = outdir+os.sep+outname+'_'+fq_base+'.2.sai'
    samfile = outdir+os.sep+outname+'_'+fq_base+'.sam'
    ...
    slurmout.write('samtools view -bf 0x2 -q 10 -@ 8 {} > {}\n\n'.format(samfile,bamfile))
...
''')
            ...
            aln=False
            #split_flag='.fastq'
            split_flag1 = '_R1_001_val_1.fq.gz'
            split_flag2 = '_R2_001_val_2.fq.gz'          
            match1 = re.compile(split_flag1)   
            match2 = re.compile(split_flag2)            
            
            # here starts the process of fq files
            genome_index = '/nv/vol190/zanglab/zw5j/data/index/bwa_index/{}/{}.fa'.format(species,species)
            ...
            slurmout.write('bdg2bw {}/{}_treat_pileup.bdg /nv/vol190/zanglab/zw5j/data/Genome/ucsc/{}/{}.chrom.sizes\n\n'.format(outdir,outname,species,species))
        
            # == merge bam files if there are multiple datasets
            if len(macs_treat)>1:
                slurmout.write('samtools merge -@ 4 {}/{}_treat.bam \\\n{}\n\n'.format(outdir,outname,' \\\n'.join(macs_treat)))    
            if len(macs_ctrl)>1:
                slurmout.write('samtools merge -@ 4 {}/{}_control.bam \\\n{}\n\n'.format(outdir,outname,' \\\n'.join(macs_ctrl)))  

            # qc for each block
            qc_out_file = slurmdir+os.sep+'{}_qc.log'.format(outname)
            slurmout.write('python 2_chipseq_qc_pe.py -i {} -o {} -s {} -b {} -f1 {} -f2 {}\n\n'.format(infile,qc_out_file,species,outname,split_flag1,split_flag2)) 
