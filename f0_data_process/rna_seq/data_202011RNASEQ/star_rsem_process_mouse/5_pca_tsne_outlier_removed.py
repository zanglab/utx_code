...
def pca_plot(df,columns,label_colors,figname,re_order=False,re_labels=False,mark_text=False):
    ...
    for ploc in np.arange(projected.shape[0]):
        ...
        plt.scatter(projected[ploc,0],projected[ploc,1],c=label_colors[columns[ploc]][0],label=plabel,s=20)
        if mark_text:
    ...
    plt.legend(handles, labels,bbox_to_anchor=[1,1],fontsize=14,frameon=False,\
...
def main():
    ...
    outdir = 'f5_PCA_tSNE_outlier_removed'
...
#     df = df[(df>1).sum(axis=1)>(df.shape[1]*.05)];print('df dim\t',df.shape)
    df = df[df.sum(axis=1)>1];print('df dim\t',df.shape)
    df = np.log2(df+1)

    outliers = ['UTX_FUSIDR_rep1','UTX_FUSIDR_rep2']
    ...
    re_order = [1,2,4,3,0]
    re_labels= {'del_cIDR':'del_cIDR',\
                'del_TPR':'del_TPR',\
                'Vector':'Vector',\
                'WT':'WT',\
                'UTX_FUSIDR':'UTX_FUSIDR',\
                'UTX_eIFIDR':'UTX_eIFIDR'}
    #re_order=False 
...
#     clustering_plot.hi_plot(np.transpose(df),df.columns,label_colors,figname)
    ...
    pca_plot(np.transpose(df),df.columns,label_colors,figname,re_order,re_labels,mark_text=True)

    figname = '{}/tpm_PCA_followed_tSNE.pdf'.format(outdir)
    pca_with_tsne(np.transpose(df),df.columns,label_colors,figname,re_order,re_labels,mark_text=True)
