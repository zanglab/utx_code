...
def compr_plot(x,y,lx,ly,thre_x,thre_y,elements,label_colors,figname,order=None,re_labels=None,mark_text=False):
    ...
    for ploc in np.arange(x.shape[0]):
        ...
        plt.scatter(x.iloc[ploc],y.iloc[ploc],c=label_colors[elements[ploc]][0],label=plabel,s=10)
        if mark_text:
            plt.axes().text(x.iloc[ploc],y.iloc[ploc],elements[ploc],fontsize=6)
...
def compr_plot_simp(x,y,lx,ly,thre_x,thre_y,elements,figname,mark_text=False):
    
    plt.figure(figsize=(4,4))
    for ploc in np.arange(x.shape[0]):
        plt.scatter(x.iloc[ploc],y.iloc[ploc],s=15,color='k')
        if mark_text:
            plt.axes().text(x.iloc[ploc],y.iloc[ploc],elements[ploc],fontsize=6)
    # ==== re-order the legends, if needed
    if thre_x:
...
def main(infile,outdir):
    ...
    df = pd.read_csv(infile,index_col=0)#;print(df)

    figname = '{}/qc_compr_uniqReads_vs_mappingRate.pdf'.format(outdir)
    x,y = df['%mapping'],df['uniq reads']/1000000,
    lx,ly = 'mapping rate(%)','non-redundant reads (M)',
    thre_x,thre_y = .3,.1,
    compr_plot_simp(x,y,lx,ly,thre_x,thre_y,df.index,figname)
