...
def plot_DCI_cor_RPKM(df,stats_df,outdir,factor,count_type,compr_type,batch,hm,p_thre,fc_thre):
    ...
    if dci_col in df.columns:
        ...
        expr_col_p = '{}_padj'.format(compr_type)
        kept_col = df[(df[expr_col_p]<p_thre)&(np.abs(df[expr_col])>np.log2(fc_thre))].index

        x = df.loc[kept_col,dci_col]
        y = df.loc[kept_col,rpkm_col]
        
        # == stats test
        slope, intercept, r_value, p_value, std_err = stats.linregress(x, y)    
        output_prename = '{}_{}_cor_DCI_{}_{}_{}'.format(factor,count_type,compr_type,batch,hm)
        stats_df.loc[output_prename,'#DEG'] = len(kept_col)
        ...
        plt.xlabel('{} {} DCI \n {} over {}'.format(batch,hm,cellType_labels[treatment],cellType_labels[control]),fontsize=12)
...
factors = ['H3K4me1','H3K4me3','H3K27ac','H3K27me3']
        ...
        for compr_type in compr_types[:]:
            for batch in ['Batch1','Batch2'][:]:
                for hm in ['H3K4me3','H3K27ac'][:]:
                    plot_DCI_cor_RPKM(combined_df,stats_df,outdir,factor,count_type,compr_type,batch,hm,p_thre,fc_thre)
