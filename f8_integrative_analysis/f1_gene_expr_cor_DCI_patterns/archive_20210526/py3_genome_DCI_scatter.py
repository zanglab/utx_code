...
def scatter_plot_compr_DCI(subdir,hm_mark,compr_type,suffix,dci_thre):
    ...
    test_file='{}/{}/{}_{}{}.bed'.format(DCI_dir,subdir,hm_mark,compr_x,suffix)
    print(test_file)
    if os.path.isfile(test_file):    
        ...
        plt.scatter(dci_df_x.loc[up_bins,'DCI'],dci_df_y.loc[up_bins,'DCI'],c='tab:red',s=3,alpha=1,rasterized=True,label='Bins w/ increased DCI in WT/Vector')
        plt.scatter(dci_df_x.loc[dn_bins,'DCI'],dci_df_y.loc[dn_bins,'DCI'],c='tab:blue',s=3,alpha=1,rasterized=True,label='Bins w/ decreased DCI in WT/Vector')
...
def plot_box_figs(subdir,hm_mark,suffix,selected_bins,color,title,dci_thre,num_DCI_bins_df):
    ...
    if os.path.isfile(test_file):   
        ...
        plt.ylabel('DCI of {}'.format(hm_mark),fontsize=15)
        # plt.ylim([-1,2])
        plt.axhline(y=0,c='k',lw=1)
        plt.title(title)
        # plt.legend(fontsize=16,borderaxespad=0.2,labelspacing=.2,handletextpad=0.2,handlelength=1,loc="upper right",frameon=False)
        plt.savefig('{}/{}/box_{}_{}_bins{}_dci{}.png'.format(outdir,subdir,hm_mark,title.split()[2],suffix,dci_thre),\
                    bbox_inches='tight',pad_inches=0.1,dpi=600,transparent=True)
        plt.show()
        plt.close()
