...
def plot_figs(box_vals,outdir_tmp,hm,compr_figname,peak_file,avg_thre):
...
#         print(hm,compr_pos,s,p)
    ...
    plt.axes().set_xticklabels(['w/ UTX binding and \n increased k4me1','w/ UTX binding and \n NOT increased k4me1'],rotation=30,ha='center')    
    ...
    plt.savefig(outdir_tmp+os.sep+'avgRPKM{}_{}_{}_on_{}.png'.format(avg_thre,hm,compr_figname,peak_file),bbox_inches='tight',pad_inches=0.1,dpi=600,transparent=True)
    plt.show()
    plt.close()
...
outdir = 'f2b_utx_binding_DCI_figs_with_wo_k4_FC1.5'
os.makedirs(outdir,exist_ok=True)

master_file_dir='{}/f7_chipseq/f7_diff_binding_on_UTX_sep_k4me1/f0_data_integration_v3_RPKM/f3_combined_data_RPKM'.format(project_dir)
...
for avg_thre in avg_thres:
    for peak_file in peak_files[:1]:
        ...
        master_df_tmp = master_df[(master_df[k4me1_log2fc_col]> np.log2(fc_thre)) & (master_df[k4me1_log2avg_col]>np.log2(avg_thre))] 
        master_df_tmp.to_csv('{}/{}.csv'.format(outdir,avg_thre))
        continue
        ...
        for subdir in subdirs[:2]:
            ...
            for dci_file_basename in name_match_df.index[:]:
                ...
                if os.path.isfile(dci_file):
                    ...
                    utx_binding_dci = utx_df.loc[master_df.index].dropna()['info']
                    utx_binding_dci = [float(ii) for ele in utx_binding_dci for ii in ele.split(',')]
                    utx_with_increased_k4 = utx_df.loc[master_df_tmp.index].dropna()['info']
                    utx_with_increased_k4 = [float(ii) for ele in utx_with_increased_k4 for ii in ele.split(',')]
                    utx_without_increased_k4 = utx_df.loc[master_df.index.difference(master_df_tmp.index)].dropna()['info']
                    utx_without_increased_k4 = [float(ii) for ele in utx_without_increased_k4 for ii in ele.split(',')]
                    # utx_without_increased_k4 = utx_df.loc[!master_df_tmp.index].dropna()['info']
                    # utx_with_increased_k4 = [float(ii) for ele in utx_with_increased_k4 for ii in ele.split(',')]
                    box_vals = [utx_with_increased_k4,utx_without_increased_k4]
                    print(dci_file,[len(ii) for ii in box_vals])
                    plot_figs(box_vals,outdir_tmp,hm,compr_figname,peak_file,avg_thre)
