        ...
        if chrom not in regions:
            ...
            for island in islands[chrom]:              
                ...
                if s > e:
                    ...
                    col_data = [float(regions[chrom][i][2].split()[0]) for i in np.arange(e,s)]#  HERE IS THE ADDED COLUMN
                    # print(island[2],col_data)
                    # col_info = np.mean(col_data)
                    col_info = ','.join(map(str,col_data))
                    island[2] = island[2]+'\t'+str(col_info)
                    if chrom not in overlapped:
                        overlapped[chrom] = []
                    overlapped[chrom].append(island)
                else:
    ...
    with open(infile,'r') as inf:
        ...
        while line:
            ...
            if not re.match("#",line):
                ...
                idd = '{}_{}_{}'.format(chrom,start,end)
                #strand = sline[5]
                ...
                if 1:
                    if chrom in chroms:
                        if chrom not in regions1:
                            regions1[chrom]=[]
                        regions1[chrom].append([max(0,start-expand),end+expand,'\t'.join(sline[3:]),idd])
                    else:
                        pass   
                    ...
                    if chrom in chroms:
                        if chrom not in regions2:
                            regions2[chrom]=[]
                        regions2[chrom].append([max(0,start-expand),end+expand,'\t'.join(sline[3:]),idd])
                    else:
                        pass                                        
...
def write_regions_bed3(regions1,regions2,outfile):
    ...
    with open(outfile,'w') as outf:
        outf.write('{}\t{}\t{}\t{}\t{}\t{}\n'.format('#chrom','start','end','IfOverlap','id','info'))
        for chrom in regions1:
            for region in regions1[chrom]:
                #idd = '{}_{}_{}'.format(chrom,region[0],region[1])
                outf.write('{}\t{}\t{}\t1\t{}\t{}\n'.format(chrom,region[0],region[1],region[3],region[2].split('\t')[-1]))
        for chrom in regions2:
            for region in regions2[chrom]:
                #idd = '{}_{}_{}'.format(chrom,region[0],region[1])
                outf.write('{}\t{}\t{}\t0\t{}\t{}\n'.format(chrom,region[0],region[1],region[3],region[2].split('\t')[-1]))  
