...
for bart_dis in ['200k']:
    ...
    outdir = 'f1_figs_compr_dci_{}'.format(bart_dis)
    os.makedirs(outdir,exist_ok=True)
    for compr_name_ii in np.arange(len(compr_names))[:]:
        compr_name = compr_names[compr_name_ii]
        islands_file='{}/{}_dci_overlap_sicer_islands.bed'.format(indir,compr_name)
        ...
        # if overlapped with HM peaks/islands
        increased_with_hm_file='{}/{}_dci_overlap_sicer_increased_HM_overlapped.bed'.format(indir,compr_name)
        increased_without_hm_file='{}/{}_dci_overlap_sicer_increased_HM_NOT_overlapped.bed'.format(indir,compr_name)
        decreased_with_hm_file='{}/{}_dci_overlap_sicer_decreased_HM_overlapped.bed'.format(indir,compr_name)
        decreased_without_hm_file='{}/{}_dci_overlap_sicer_decreased_HM_NOT_overlapped.bed'.format(indir,compr_name)

        # plot the figs
        HM='H3K4me3' if re.search('K4M3',compr_name) else 'H3K27ac'
        infiles = [islands_file,increased_with_hm_file,decreased_with_hm_file]
        colors=['grey','r','b']
        compr_poses=[[0,1,'t'],[0,2,'b']]
        xticklabels=['All islands','w/ increased UTX \n w/ {}'.format(HM),'w/ decreased UTX \n w/ {}'.format(HM)]
        figname=outdir+os.sep+'{}_{}_compr_dci.pdf'.format(bart_dis,compr_name)
        plot_box_figs(infiles,colors,compr_poses,xticklabels,figname)
        

        infiles = [islands_file,increased_with_hm_file,increased_without_hm_file]
        colors=['grey','r','salmon']
        compr_poses=[[0,1,'t'],[0,2,'b']]
        xticklabels=['All islands','w/ increased UTX \n w/ {}'.format(HM),'w/ increased UTX \n w/o {}'.format(HM)]
        figname=outdir+os.sep+'{}_{}_compr_dci_increased.pdf'.format(bart_dis,compr_name)
        plot_box_figs(infiles,colors,compr_poses,xticklabels,figname)

        
        infiles = [islands_file,decreased_with_hm_file,decreased_without_hm_file]
        colors=['grey','b','lightblue']
        compr_poses=[[0,1,'t'],[0,2,'b']]
        xticklabels=['All islands','w/ decreased UTX \n w/ {}'.format(HM),'w/ decreased UTX \n w/o {}'.format(HM)]
        figname=outdir+os.sep+'{}_{}_compr_dci_decreased.pdf'.format(bart_dis,compr_name)
        plot_box_figs(infiles,colors,compr_poses,xticklabels,figname)
