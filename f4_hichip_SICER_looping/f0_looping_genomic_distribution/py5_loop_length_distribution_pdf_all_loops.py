...
def loop_length_distribution(hms,cellTypes,colors,indir,outdir,flag):
    ...
    for ii in np.arange(len(hms)):
        ...
        loop_length_all = np.append(loop_length_all,loop_length.values)
    
    sns.distplot(loop_length_all/1000,kde=False,norm_hist=False,color='k')
        
    plt.ylabel('Number of loops',fontsize=16)
...
outdir = 'f5_loop_length_distribution_pdf'
