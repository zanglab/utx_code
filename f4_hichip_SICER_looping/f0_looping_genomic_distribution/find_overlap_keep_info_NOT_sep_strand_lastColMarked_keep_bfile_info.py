...
#                     print(island)
#                     print(regions[chrom][e]);exit()
                    col_genes = [j for i in np.arange(e,s) for j in regions[chrom][i][2].split()[0].split(',')]#  HERE IS THE ADDED COLUMN
                    col_info = ','.join(set(col_genes))#  HERE IS THE ADDED COLUMN
...
def read_region_from_bed(infile,species,expand=0,skip_lines=False,write_out=None,end_col=2,region_sorted=True):
...
def write_regions_bed3(regions1,regions2,outfile):
    ...
    with open(outfile,'w') as outf:
        outf.write('{}\t{}\t{}\t{}\t{}\n'.format('#chrom','start','end','IfOverlap','id\tcount\texpected\tfdr\tClusterLabel\tClusterSize\tClusterType\tClusterNegLog10P\tClusterSummit\tOverlapped_genes'))
        for chrom in regions1:
            for region in regions1[chrom]:
                outf.write('{}\t{}\t{}\t1\t{}\n'.format(chrom,region[0],region[1],region[2]))
        for chrom in regions2:
