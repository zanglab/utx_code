...
indir = '../data_202008_new_reindex'
outdir = 'f5_data_202008_new_shared_loop'
...
for hm_name in hm_names.keys():
    ...
    pd.Series(shared_index,index=shared_index).to_csv(outdir+os.sep+'shared_index_{}.txt'.format(hm_name), header=False,index=False)
    ...
    for norm_col in norm_cols[:]:
        for kept_col in ['count','expected'][:]:            
            ...
            plt.savefig(outdir+os.sep+'shared_loops_{}_{}_NormBy_{}.pdf'.format(hm_name,kept_col,norm_col),bbox_inches='tight',pad_inches=0.1,dpi=600)
            plt.show()
            plt.close()
