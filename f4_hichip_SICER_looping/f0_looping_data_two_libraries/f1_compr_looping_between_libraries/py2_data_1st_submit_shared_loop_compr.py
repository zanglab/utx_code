...
outdir = 'f2_1st_submit_shared_loop'
os.makedirs(outdir,exist_ok=True)

sample_names = ['PCDH3HICHIP','FL3HICHIP','DEL33HICHIP','EIF3HICHIP']
# sample_names = ['PCDH4HICHIP','FL4HICHIP','DEL34HICHIP','EIF4HICHIP']
# number_of_pairs_after_duplicate_removal
norm_factors = {'PCDH3HICHIP':77696968,'FL3HICHIP':62299145,'DEL33HICHIP':72278154,'EIF3HICHIP':58191452}
...
for sample_name in sample_names:
    ...
    with open(infile) as inf:
        ...
        if len(shared_index)==0:
            ...
            shared_index = shared_index.intersection(df.index)
pd.Series(shared_index,index=shared_index).to_csv(outdir+os.sep+'shared_index.txt', header=False,index=False)
...
#         mark_pvalue(compr_pos,positions,box_vals)
    ...
    plt.savefig(outdir+os.sep+'shared_loops_{}.pdf'.format(kept_col),bbox_inches='tight',pad_inches=0.1,dpi=600)
#     plt.show()
    plt.close()
