...
def return_deg(deg_file,adjp=0.05,logfc=np.log2(1.25)):
    ...
    rep_col=[i for i in deseq_out.columns if re.search('rep',i)]
    deseq_out = deseq_out[deseq_out[rep_col].sum(axis=1)>0]
...
def return_overlapped_gene_significance(df_count,df_names,df_index,genes,target_genes,col_name):
    ...
    # fisher exact test
    va = len(overlapped)
    vb = len(genes)-len(overlapped)
    vc = len(target_genes)- len(overlapped)
    vd = 20794 - len(genes) - len(target_genes) + len(overlapped)
...
indir = project_dir+os.sep+'f4_hichip_SICER_looping/f1_diff_looping/f2_diff_looping_high_thre/'
outdir = 'f3b_gene_cor_diff_looping_high_threshold'
